...
public class TaskHandler implements HttpHandler {
    ...
    public final static String MAPPING = "/admin/tasks";
    ...
    private void doGet(String task, HttpServerExchange exchange) {
        if (Strings.isNullOrEmpty(task) || "/".equalsIgnoreCase(task)) {
            try (final PrintWriter output = new PrintWriter(exchange.getOutputStream(), true)) {
    ...
    private void doPost(String taskKey, HttpServerExchange exchange) {
        final Task task = tasks.get(taskKey);
        if (task != null) {
            exchange.getResponseHeaders().put(Headers.CONTENT_TYPE, MediaType.PLAIN_TEXT_UTF_8.toString());
    ...
    public void handleRequest(HttpServerExchange exchange) throws Exception {
        ...
        String task = exchange.getRequestPath().substring(MAPPING.length());
