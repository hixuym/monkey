...
public interface Context {
    ...
    /**
     * Impl is used to hide stuff that a user should not see on code completion. Internal stuff like setting routes
     * should go here.
     */
    ...
    /**
     * Used to enable or disable usage of X-Forwarded-For header in getRemoteAddr(). Can be set in application.conf to
     * true or false. If not set it's assumed to be false;
     */
    String GIZMO_PROPERTIES_X_FORWARDED_FOR = "gizmo.x_forwarded_for_enabled";

    /**
     * The Content-Type header field indicates the media type of the request body sent to the recipient. E.g. {@code
     ...
     *
     * To honour the X-Forwarded-For flag make sure you set "ninja.ninja.x_forwarded_for_enabled=true" in your
     * application.conf. Default behavior is NOT to take X-Forwarded-For flag into account.
     *
     * @return a <code>String</code> containing the IP address of the client that sent the request. Takes into account
     * X-Forwarded-For header if configured to do so.
     */
     ...
     *
     * If your route only defines /app it will work as the requestpath will return only "/app". A context path is not
     * returned.
     *
     * It does NOT decode any parts of the url.
     *
     * Interesting reads: - http://www.lunatech-research.com/archives/2009/02/03/ what-every-web-developer-must-know-about-url-encoding
    ...
    /**
     * Returns the flash cookie. Flash cookies only live for one request. Good uses are error messages to display.
     * Almost everything else is bad use of Flash Cookies. A FlashScope is usually not signed. Don't trust the content.
     *
    ...
    /**
     * Returns the client side session. It is a cookie. Therefore you cannot store a lot of information inside the
     * cookie. This is by intention.
     *
     * If you have the feeling that the session cookie is too small for what you want to achieve thing again. Most
     * likely your design is wrong.
     *
     ...
     *
     * That means: - when running on root the context path is empty - when running on context there is NEVER a trailing
     * slash
     *
     * We conform to the following rules: Returns the portion of the request URI that indicates the context of the
     * request. The context path always comes first in a request URI. The path starts with a "/" character but does not
     * end with a "/" character. For servlets in the default (root) context, this method returns "". The container does
     * not decode this string.
     *
     ...
     * case of form submissions, may be a form parameter. <p> When the parameter is multivalued, returns the first
     * value. <p> The parameter is decoded by default.
     *
    ...
    /**
     * Get the parameter with the given key from the request. The parameter may either be a query parameter, or in the
     * case of form submissions, may be a form parameter. <p> The parameter is decoded by default.
     *
     * @param name The key of the parameter
    ...
    /**
     * Same like {@link #getParameter(String)}, but returns given defaultValue instead of null in case parameter cannot
     * be found.
     *
    ...
    /**
     * Same like {@link #getParameter(String, String)}, but converts the parameter to Integer if found.
     *
     * The parameter is decoded by default.
     *
     * @param name         The name of the post or query parameter
     ...
     *
     * The parameter is read from the multipart stream, using the FileProvider declared on the route method, class.
     *
     * @param name The name of the post or query parameter
     * @return The value of the parameter or null if not found.
     */
    ...
    /**
     * Same like {@link #getParameter(String, String)}, but converts the parameter to Class type if found.
     *
     * The parameter is decoded by default.
     *
     * @param name         The name of the post or query parameter
     ...
     * @return the numeric path parameter, or null of no such path parameter is defined, or if it cannot be parsed to
     * int
     */
    ...
    /**
     * This will give you the request body nicely parsed. You can register your own parsers depending on the request
     * type.
     *
     * Have a look at {@link BodyParserEngine} {@link BodyParserEngineJson} and {@link BodyParserEngineManager}
     *
    ...
    /**
     * Finalizing the headers copies all stuff into the headers. It of course also handles Gizmo session and Flash
     * information.
     *
     ...
     * After finalizing the headers you can access the responseStreams.
     *
     * This method does not set any Gizmo session of flash information. Eg. When serving static assets this is the
     * method you may want to use. Otherwise you'd get a race condition with a lot of requests setting scopes and
    ...
    /**
     * Check if request is of type multipart. Important when you want to process uploads for instance.
     *
    ...
    /**
     * Get the content type that is acceptable for the client. (in this order : {@see Result.TEXT_HTML} > {@see
     * Result.APPLICATION_XML} > {@see Result.APPLICATION_JSON} > {@see Result.TEXT_PLAIN} > {@see
     * Result.APPLICATION_OCTET_STREAM} level- or quality-parameter are ignored with this method.) E.g. Accept:
     ...
     *
     * @return one of the {@see Result} mime types that is acceptable for the client or {@see Result.TEXT_HTML} if not
     * set
     * @see <a href="http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html" >http://www.w3.org/Protocols/rfc2616/rfc2616-sec14.html</a>
     */
     ...
     *
     * The Accept-Encoding request-header field is similar to Accept, but restricts the content-codings that are
    ...
    /**
     * Get the language that is acceptable for the client. E.g. Accept-Language: da, en-gb;q=0.8, en;q=0.7
     *
     * The Accept-Language request-header field is similar to Accept, but restricts the set of natural languages that
     * are preferred as a response to the request.
     *
    ...
    /**
     * Get the charset that is acceptable for the client. E.g. Accept-Charset: iso-8859-5, unicode-1-1;q=0.8
     *
     * The Accept-Charset request-header field can be used to indicate what character sets are acceptable for the
     * response. This field allows clients capable of understanding more comprehensive or special- purpose character
     * sets to signal that capability to a server which is capable of representing documents in those character sets.
     *
     ...
     *
     * @return a <code>String</code> specifying the name of the method with which this request was made (eg GET, POST,
     * PUT...)
     */
    String getMethod();
    ...
    /**
     * Gets an attribute value previously set by {@link #setAttribute}. <p> This is a convenience method, equivalent
     * to:
     * <pre><code>
    ...
    /**
     * Check to see if the request content type is JSON. <p> Checks to see if the request content type has been set
     * application/json </p>
     *
    ...
    /**
     * Check to see if the request content type is XML. <p> Checks to see if the request content type has been set
     * application/xml </p>
     *
