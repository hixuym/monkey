...
public class DiagnosticErrorRenderer {
  ...
  static public void tryToRender(Context context, Result result, DiagnosticError diagnosticError,
  ...
  private DiagnosticErrorRenderer appendHeader(Context context,
    ...
    if (result != null) {
      ...
      s.append(" for request '").append(context.getMethod()).append(" ")
          .append(context.getRequestPath()).append("'\n");

      // append info about the route itself
      if (context.getRoute() != null) {
        Route route = context.getRoute();
        s.append("<br />In controller method '").append(route.getResourceClass().getCanonicalName())
            .append(".").append(route.getResourceMethod().getName()).append("'\n");
      }
  ...
  private DiagnosticErrorRenderer appendTabsBegin(String[] names) throws IOException {
    ...
    for (int i = 0; i < names.length; i++) {
      ...
      s.append("><a href=\"#tab").append(i).append("\">").append(escape(names[i]))
          .append("</a></li>\n");
    }
  ...
  private DiagnosticErrorRenderer appendFooter() throws IOException {
    // embed jquery
    s.append("<script type='text/javascript'>").append(getResource("jquery-1.11.1.min.js"))
        .append("</script>");
    // diagnostic javascript
    s.append("<script type='text/javascript'>").append(getResource("diagnostic.js"))
        .append("</script>");
  ...
  private DiagnosticErrorRenderer appendContext(Context context) throws IOException {
    ...
    if (context.getRoute() != null) {
      ...
      appendNameValue(s, "Controller method",
          route.getResourceClass().getCanonicalName() + "." + route.getResourceMethod().getName()
              + "()");
    ...
    if (context.getFlashScope() != null && !context.getFlashScope().getCurrentFlashCookieData()
        .isEmpty()) {
      for (Map.Entry<String, String> sessionEntry : context.getFlashScope()
    ...
    if (attributes != null && !attributes.isEmpty()) {
      for (Map.Entry<String, Object> entry : attributes.entrySet()) {
        appendNameValue(s, entry.getKey(),
  ...
  private DiagnosticErrorRenderer appendResponse(Result result) throws IOException {
    ...
    appendNameValue(s, "Json View",
        (result.getJsonView() != null ? result.getJsonView().getCanonicalName() : "None"));
  ...
  private void appendNoValues(StringBuilder sb, String title) throws IOException {
    sb.append("<pre style=\"border-bottom: 0px;\"><span style=\"position: absolute; left: 45px;\">")
        .append(escape(title)).append("</span></pre><br/>");
  }
  ...
  private DiagnosticErrorRenderer appendSourceSnippet(URI sourceLocation,
    ...
    if (sourceLines != null) {
      ...
      for (int i = 0; i < sourceLines.size(); i++) {
        ...
        s.append("<span class=\"").append(cssClass).append("\">").append(lineNumber)
            .append("</span>");
        s.append("<span class=\"")
            .append("route")
            .append("\">")
  ...
  private DiagnosticErrorRenderer appendThrowable(Throwable throwable) throws IOException {
    ...
    if (throwable != null) {
      s.append("<pre><span class=\"stacktrace\">")
          .append(escape(throwableStackTraceToString(throwable))).append("</span></pre>\n");
    } else {
