...
public class ParamParsers {
    ...
    public Size parseParameter(String field, String parameterValue, Validation validation) {
      if (Strings.isNullOrEmpty(parameterValue) || validation.hasViolation(field)) {
        ...
        try {
          ...
          validation
              .addViolation(new ConstraintViolation(SIZE_KEY, field, SIZE_MESSAGE, parameterValue));
          return null;
        }
      }
    }
      ...
      if (Strings.isNullOrEmpty(parameterValue) || validation.hasViolation(field)) {
        ...
        try {
          ...
          validation.addViolation(
              new ConstraintViolation(DURATION_KEY, field, DURATION_MESSAGE, parameterValue));
          return null;
        }
      }
    }
  ...
  public static class GenericEnumParamParser<E extends Enum<E>> implements ParamParser<E> {
    ...
    public E parseParameter(String field, String parameterValue, Validation validation) {
      if (Strings.isNullOrEmpty(parameterValue) || validation.hasViolation(field)) {
        ...
        validation.addViolation(new ConstraintViolation(
            ENUM_KEY, field, ENUM_MESSAGE,
            new Object[]{parameterValue, getParsedType().getName()}));
        return null;
      }
    }
  ...
  private static final DateTimeFormatter LOCAL_DATE_TIME_FORMATER = DateTimeFormatter
      .ofPattern("yyyy-MM-dd HH:mm:ss");
  private static final DateTimeFormatter LOCAL_DATE_FORMATER = DateTimeFormatter
    ...
    public LocalDate parseParameter(String field, String parameterValue, Validation validation) {
      if (Strings.isNullOrEmpty(parameterValue) || validation.hasViolation(field)) {
        ...
        try {
          ...
          validation
              .addViolation(new ConstraintViolation(DATE_KEY, field, DATE_MESSAGE, parameterValue));
          return null;
        }
      }
    }
  ...
  public static class LocalDateTimeParamParser implements ParamParser<LocalDateTime> {
    ...
    @Override
    public LocalDateTime parseParameter(String field, String parameterValue,
