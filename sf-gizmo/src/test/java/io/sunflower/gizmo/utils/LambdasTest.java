...
public class LambdasTest {
  ...
  public void staticMethodReference() throws Exception {
    ...
    assertThat(lambdaInfo.getKind(), is(Lambdas.Kind.STATIC_METHOD_REFERENCE));
  ...
  public void specificInstanceMethodReference() throws Exception {
    ...
    assertThat(lambdaInfo.getKind(), is(Lambdas.Kind.SPECIFIC_INSTANCE_METHOD_REFERENCE));
  ...
  public void anyInstanceMethodReference() throws Exception {
    ...
    assertThat(lambdaInfo.getKind(), is(Lambdas.Kind.ANY_INSTANCE_METHOD_REFERENCE));
  ...
  public void anonymousMethodReference() throws Exception {
    ResourceMethods.ResourceMethod1<Context> lambda = (Context context) -> Results.html()
        .renderRaw("".getBytes(StandardCharsets.UTF_8));

    Lambdas.LambdaInfo lambdaInfo = Lambdas.reflect(lambda);

    assertThat(lambdaInfo.getKind(), is(Lambdas.Kind.ANONYMOUS_METHOD_REFERENCE));
    ...
    assertThat(serializedLambda.getInstantiatedMethodType(),
        is("(Lio/sunflower/gizmo/Context;)Lio/sunflower/gizmo/Result;"));
