...
import io.sunflower.gizmo.validation.JSR303Validation;
import io.sunflower.gizmo.validation.NumberValue;
...
public class ResourceMethodInvokerTest {
  ...
  public void setUp() throws Exception {
    this.configuration = Mockito.spy(new GizmoConfiguration());
    this.lang = new LangImpl(this.configuration);
...
//        when(context.getHeader("param1")).thenReturn(null);
  ...
  public void enumArrayParamMultipleShouldBeParsed() throws Exception {
    when(context.getParameterValues("param1"))
        .thenReturn(Arrays.asList("Blue", "Indigo", "Violet"));
    create("enumArrayParam").invoke(mockController, context);
    verify(mockController)
        .enumArrayParam(new Rainbow[]{Rainbow.Blue, Rainbow.Indigo, Rainbow.Violet});
  }
  ...
  public void customDateFormatParamShouldBeParsedToDate() throws Exception {
    ...
    verify(mockController).dateParam(Dates.asUtilDate(LocalDateTime.of(2015, 1, 15, 0, 0)));
  }
  ...
  public void customDateFormatParamWithOptionalShouldBeParsedToDate() throws Exception {
    ...
    verify(mockController)
        .dateParamWithOptional(Optional.of(Dates.asUtilDate(LocalDateTime.of(2015, 1, 15, 0, 0))));
  }
...
//        when(context.getParameter("param1")).thenReturn("hello");
  ...
  public void customArgumentExtractorWithOptionalAndGuiceShouldBeInstantiated() {
    ...
    verify(mockController)
        .guiceArgumentExtractorWithOptional(Optional.of("dep:bar:java.lang.String"));
  }
...
//        when(context.getParameter("param1")).thenReturn(null);
  ...
  public void optionalDateParam() {
    ...
    verify(mockController)
        .optionalDateParam(Optional.of(Dates.asUtilDate(LocalDateTime.of(2015, 1, 15, 0, 0))));
  }
  ...
  public void validationShouldBeAppliedInCorrectOrderPreFail() {
    ...
    assertEquals("validation.number.min.violation",
        validation.getViolations("param1").get(0).getMessageKey());
  }
  ...
  private void doCheckValidationFailedTranslationFr(Context context) {
    ...
    assertEquals(this.context.getValidation().getViolations().get(0).getDefaultMessage(),
  ...
  private void doCheckValidationFailedTranslationEn(Context context) {
    assertTrue(context.getValidation().hasViolations());
    assertEquals(context.getValidation().getViolations().size(), 1);
    assertEquals(context.getValidation().getViolations().get(0).getDefaultMessage(),
  ...
  public interface MockController {
    ...
    Result flash(FlashScope flash);

    Result param(@Param("param1") String param1);

    Result pathParam(@PathParam("param1") String param1);

    Result sessionParam(@SessionParam("param1") String param1);

    Result attribute(@Attribute("param1") Dep param1);

    Result header(@Header("param1") String param1);

    Result headers(@Headers("param1") String[] param1);

    Result integerParam(@Param("param1") Integer param1);

    Result intParam(@Param("param1") int param1);

    Result shortParam(@Param("param1") Short param1);

    Result primShortParam(@Param("param1") short param1);

    Result characterParam(@Param("param1") Character param1);

    Result charParam(@Param("param1") char param1);

    Result byteParam(@Param("param1") Byte param1);

    Result primByteParam(@Param("param1") byte param1);

    Result booleanParam(@Param("param1") Boolean param1);

    Result booleanParamWithOptional(@Param("param1") Optional<Boolean> param1);

    Result primBooleanParam(@Param("param1") boolean param1);

    Result longParam(@Param("param1") Long param1);

    Result primLongParam(@Param("param1") long param1);

    Result floatParam(@Param("param1") Float param1);

    Result primFloatParam(@Param("param1") float param1);

    Result doubleParam(@Param("param1") Double param1);

    Result primDoubleParam(@Param("param1") double param1);

    Result enumParam(@Param("param1") Rainbow param1);

    Result enumCsvParam(@Param("param1") Rainbow[] param1);

    Result enumArrayParam(@Params("param1") Rainbow[] param1);

    Result noArgArgumentExtractor(@NoArg String param1);

    Result classArgArgumentExtractor(@ClassArg String param1);

    Result guiceArgumentExtractor(@GuiceAnnotation(foo = "bar") String param1);

    Result guiceArgumentExtractorWithOptional(
        @GuiceAnnotation(foo = "bar") Optional<String> param1);

    Result multiple(@Param("param1") String param1, @PathParam("param2") int param2,
        Context context, Session session);

    Result optionalSessionParam(@SessionParam("param1") Optional<String> param1);

    Result optionalAttribute(@Attribute("param1") Optional<Dep> param1);

    Result optionalHeader(@Header("param1") Optional<String> param1);

    Result optionalHeaders(@Headers("param1") Optional<String[]> param1);

    Result optionalParam(@Param("param1") Optional<String> param1);

    Result optionalIntegerParam(@Param("param1") Optional<Integer> param1);

    Result optionalLongParam(@Param("param1") Optional<Long> param1);

    Result optionalShortParam(@Param("param1") Optional<Short> param1);

    Result optionalEnumParam(@Param("param1") Optional<Rainbow> param1);

    Result optionalEnumArrayParam(@Param("param1") Optional<Rainbow[]> param1);

    Result optionalDateParam(@Param("param1") Optional<Date> param1);

    Result optionalBody(Optional<Object> body);

    Result requiredInt(@Param("param1") @NumberValue(min = 10) int param1);

    Result badValidator(@Param("param1") @NumberValue(min = 10) String param1);

    Result badValidatorWithOptional(
        @Param("param1") @NumberValue(min = 10) Optional<String> param1);

    Result body(Object body);

    Result bodyWithOptional(Optional<Object> body);
    ...
    Result dateParamWithOptional(@Param("param1") Optional<Date> param1);

    Result needingInjectionParamParser(@Param("param1") Dep param1);

    Result needingInjectionParamParserArray(@Params("param1") Dep[] paramsArray);
  }
    ...
    public Date parseParameter(String field, String parameterValue, Validation validation) {
      try {
        return parameterValue == null ? null
            : new SimpleDateFormat(DATE_FORMAT).parse(parameterValue);
  ...
  public static class NeedingInjectionParamParser implements ParamParser<Dep> {
    ...
    @Inject
    GizmoConfiguration properties;

    @Override
    public Dep parseParameter(String field, String parameterValue, Validation validation) {
      return new Dep(properties.getCookiePrefix() + "_" + parameterValue);
    }
  ...
  /**
   * Argument extractor that has a complex constructor for Guice. It depends on some other
   * dependency (dep), plus the annotation that was on the parameter, and the class of the
   * parameter.
   */
