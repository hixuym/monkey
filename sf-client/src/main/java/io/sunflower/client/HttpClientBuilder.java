...
import io.sunflower.client.proxy.NonProxyListProxyRoutePlanner;
...
public class HttpClientBuilder {
    ...
    /**
     * @return an {@link ConfiguredCloseableHttpClient}
     */
     ...
     * <p>
     * Intended for use by subclasses to inject HttpClientBuilder configuration. The default
    ...
    /**
     * Map the parameters in {@link HttpClientConfiguration} to configuration on a {@link
    ...
    protected ConfiguredCloseableHttpClient createClient(
        ...
        final String cookiePolicy =
                configuration.isCookiesEnabled() ? CookieSpecs.DEFAULT : CookieSpecs.IGNORE_COOKIES;
        ...
        final Integer connectionRequestTimeout = (int) configuration.getConnectionRequestTimeout()
                .toMilliseconds();
        ...
        final HttpRequestRetryHandler retryHandler = configuration.getRetries() == 0
                ? NO_RETRIES
                : (httpRequestRetryHandler == null ? new DefaultHttpRequestRetryHandler(
        ...
        if (proxy != null) {
            ...
            builder
                    .setRoutePlanner(new NonProxyListProxyRoutePlanner(httpHost, proxy.getNonProxyHosts()));
    ...
    /**
     * Create a user agent string using the configured user agent if defined, otherwise using a
     * combination of the environment name and this client name
     *
    ...
    /**
     * Create a InstrumentedHttpClientConnectionManager based on the HttpClientConfiguration. It sets
     * the maximum connections per route and the maximum total connections that the connection manager
     * can create
     *
     ...
     *
     * @return a {@code Credentials} instance, either {{@link UsernamePasswordCredentials} or {@link
     * NTCredentials}}
     */
    protected Credentials configureCredentials(AuthConfiguration auth) {

        if (null != auth.getCredentialType() && auth.getCredentialType()
                .equalsIgnoreCase(AuthConfiguration.NT_CREDS)) {
            return new NTCredentials(auth.getUsername(), auth.getPassword(), auth.getHostname(),
                    auth.getDomain());
        } else {
