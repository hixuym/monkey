...
/**
 * @author Michael
 * Created at: 2019/2/18 13:44
 * @param <C> The extension context
 */
public abstract class AbstractMonkeyExtension<C> implements TestTransactionInterceptor {
     ...
     */
    protected void beforeClass(C context, Class<?> testClass, @Nullable MonkeyTest testAnnotation) {
        if (testAnnotation != null) {
            this.rollback = testAnnotation.rollback();

            final ApplicationContextBuilder builder = MonkeyApplicationContext.build();
                        ...
                        if (loader != null) {
                            ...
                            if (inputStream != null) {
                                ...
                                try {
                                    ...
                                    throw new RuntimeException("Error loading property source reference for @MonkeyTest: " + filename);
                                } finally {
                                    try {
                                        inputStream.close();
                                    } catch (IOException e) {
    ...
    /**
     * Executed before tests within each test method.
    ...
    /**
     * start monkey application context.
     */
    ...
    /**
     * check test class presented.
     * @param requiredTestClass test class
    ...
    private Map<String, PropertySourceLoader> readPropertySourceLoaderMap() {
        Map<String, PropertySourceLoader> loaderMap = AbstractMonkeyExtension.loaderMap;
        if (loaderMap == null) {
            loaderMap = new HashMap<>();
            AbstractMonkeyExtension.loaderMap = loaderMap;
