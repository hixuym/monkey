...
public class ExtensionLoaderTest extends TestCase {
    ...
    public void testExtensionNormal() {
        // 单例模式下只会构造一次实例
        Assert.assertEquals(1, ExtensionLoader.of(SpiTestInterface.class).getExtension("spitest").spiHello());
        Assert.assertEquals(1, ExtensionLoader.of(SpiTestInterface.class).getExtension("spitest").spiHello());

        // 多例模式下在每次获取的时候进行实例化
        Assert.assertEquals(1, ExtensionLoader.of(SpiPrototypeInterface.class).getExtension("spiPrototypeTest").spiHello());
        Assert.assertEquals(2, ExtensionLoader.of(SpiPrototypeInterface.class).getExtension("spiPrototypeTest").spiHello());

        // 手动添加实现类
        Assert.assertEquals(1, ExtensionLoader.of(SpiPrototypeInterface.class).getExtensions("").size());
        ...
        Assert.assertEquals(2, ExtensionLoader.of(SpiPrototypeInterface.class).getExtensions("").size());
