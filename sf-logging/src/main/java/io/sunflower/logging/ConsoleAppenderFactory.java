 ...
 * <td>{@code type}</td> <td><b>REQUIRED</b></td> <td>The appender type. Must be {@code console}.</td> </tr> <tr>
 * <td>{@code threshold}</td> <td>{@code ALL}</td> <td>The lowest level of events to print to the console.</td> </tr>
 * <tr> <td>{@code timeZone}</td> <td>{@code UTC}</td> <td>The time zone to which event timestamps will be
 * converted.</td> </tr> <tr> <td>{@code target}</td> <td>{@code stdout}</td> <td> The name of the standard stream to
 * which events will be written. Can be {@code stdout} or {@code stderr}. </td> </tr> <tr> <td>{@code logFormat}</td>
