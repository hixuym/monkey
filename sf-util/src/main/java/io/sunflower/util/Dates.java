...
public class Dates {
    ...
    public static Date asUtilDate(long mills) {
        return Date.from(Instant.ofEpochMilli(mills));
    }

    /**
     * Creates a {@link Date} from various date objects. Is null-safe. Currently supports: <ul> <li>{@link Date}
     * <li>{@link java.sql.Date} <li>{@link java.sql.Timestamp} <li>{@link LocalDate} <li>{@link LocalDateTime}
     * <li>{@link ZonedDateTime} <li>{@link Instant} </ul>
     *
